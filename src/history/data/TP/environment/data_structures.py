"""
高效数据结构工具集：环形缓冲区和双头索引数组
包含三个核心类：
1. RingBuffer - 定长循环缓冲区
   - 特性：支持两端插入/删除，空间不足时自动覆盖或报错
   - 适用场景：实时数据流处理、滑动窗口计算
   - 优势：O(1)时间复杂度操作，内存预分配
2. DoubleHeaderNumpyArray - 双层级标头矩阵
   - 特性：支持 (主头, 副头) 二维索引访问
     (如：['position', 'x'], ['velocity', 'y'])
   - 适用场景：多维传感器数据、复杂特征矩阵
   - 优势：类Pandas多级索引的高效numpy实现
3. SingleHeaderNumpyArray - 单层标头向量（辅助类）
   - 特性：通过属性式访问列数据（如：obj.velocity）
   - 设计目的：简化双头数组的层级访问
"""
